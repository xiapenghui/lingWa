<template>
  <el-dialog v-dialogDrag :close-on-click-modal="false" :visible.sync="personShow" :before-close="personClose" title="人员信息表" width="70%" height="50%">
    ...
    <pagination v-show="logTotal > 0" :total="logTotal" :current.sync="paginationSearchPerson.PageIndex" :size.sync="paginationSearchPerson.PageSize" @pagination="getLogList" />
  </el-dialog>
</template>
...
export default {
  ...
  methods: {
    ...
    // 分页
    table_index(index) {
      return (this.paginationSearchPerson.PageIndex - 1) * this.paginationSearchPerson.PageSize + index + 1
    }
