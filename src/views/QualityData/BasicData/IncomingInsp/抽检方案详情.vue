  ...
  methods: {
    ...
    handleBan(row) {
      ...
      this.$confirm(statusTitle, status, {
        ...
        QuaDetailModifyStatus(params).then(res => {
          if (res.IsPass === true) {
            this.$message({
              type: 'success',
              message: res.MSG
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.dialogType === 'edit') {
            ...
            params.RuleCode = this.$route.query.RuleCode
            QuaDetailModify(params).then(res => {
              if (res.IsPass === true) {
                this.$message({
                  type: 'success',
    ...
    handleDelete(row) {
      this.$confirm(this.$t('permission.errorInfo'), this.$t('permission.errorTitle'), {
        ...
        .then(() => {
          QuaDetailDelete({ TaskNum: row.TaskNum }).then(res => {
            if (res.IsPass === true) {
              this.$message({
                type: 'success',
