  ...
  <div class="app-container">
    <div class="search">
      <el-row :gutter="20">
        <el-col :span="6">
          ...
          <el-col :span="16"><el-input v-model.trim="pagination.SpareNum" placeholder="备件编号" clearable /></el-col>
        </el-col>
        <el-col :span="6">
          <el-col :span="8">
          ...
          <el-col :span="16"><el-input v-model.trim="pagination.Name" placeholder="备件名称" clearable /></el-col>
        </el-col>
        <el-col :span="4">
          <el-col :span="24">
    ...
    >
      <el-form ref="ruleForm" v-loading="editLoading" :model="ruleForm" :rules="rules" label-width="100px" label-position="left">
        <el-form-item label="备件编号" prop="SpareNum"><el-input v-model.trim="ruleForm.SpareNum" placeholder="备件编号" clearable /></el-form-item>
        <el-form-item label="备件名称" prop="Name"><el-input v-model.trim="ruleForm.Name" placeholder="备件名称" clearable /></el-form-item>

        <el-form-item label="单位">
          <el-select v-model="ruleForm.SpareUnit" placeholder="请选择" style="width: 100%" clearable>
            <el-option v-for="item in UnitTextList" :key="item.value" :label="item.text" :value="item.value" />
          </el-select>
        </el-form-item>

        <el-form-item label="描述"><el-input v-model.trim="ruleForm.Description" placeholder="备件描述" type="textarea" clearable /></el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      UnitTextList: [], // 单位下拉
      rules: {
        SpareNum: [{ required: true, message: '请输入备件编号', trigger: 'blur' }],
  ...
  watch: {
    ...
    'ruleForm.SpareNum': function(val) {
      if (val === '' || val === undefined) {
        return
      } else {
        this.ruleForm.SpareNum = this.filterInput(val)
      }
    },
  ...
  methods: {
    ...
    handleBan(row) {
      ...
      this.$confirm(statusTitle, status, {
        ...
        EquSpareModifyStatus(params).then(res => {
          if (res.IsPass === true) {
            this.$message({
              type: 'success',
              message: res.MSG
    ...
    getList() {
      ...
      EquSpareList(this.pagination).then(res => {
    ...
    handleDelete(row) {
      this.$confirm(this.$t('permission.errorInfo'), this.$t('permission.errorTitle'), {
        ...
        .then(() => {
          EquSpareDelete({ SpareCode: row.SpareCode }).then(res => {
            if (res.IsPass === true) {
              this.$message({
                type: 'success',
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.dialogType === 'edit') {
            ...
            EquSpareModify(params).then(res => {
              if (res.IsPass === true) {
                this.$message({
                  type: 'success',
            ...
            EquSpareAdd(this.ruleForm).then(res => {
              if (res.IsPass === true) {
                this.$message({
                  type: 'success',
