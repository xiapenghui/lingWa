  ...
  <div class="app-container">
    ...
    >
      <el-form ref="ruleForm" v-loading="editLoading" :model="ruleForm" :rules="rules" label-width="120px" label-position="left">
        ...
        <el-form-item label="设备保养项编号" prop="MtItemsNum"><el-input v-model.trim="ruleForm.MtItemsNum" placeholder="设备保养项编号" clearable /></el-form-item>
        <el-form-item label="设备保养项名称" prop="MtItemsName"><el-input v-model.trim="ruleForm.MtItemsName" placeholder="设备保养项名称" clearable /></el-form-item>
        <el-form-item label="设备保养工具" prop="MtTool"><el-input v-model.trim="ruleForm.MtTool" placeholder="设备保养工具" clearable /></el-form-item>
        <el-form-item label="保养方法"><el-input v-model.trim="ruleForm.MtMethod" placeholder="保养方法" type="textarea" /></el-form-item>
      </el-form>
...
import { GetValuePair, EquMtItemsList, EquMtItemsAdd, EquMtItemsDelete, EquMtItemsModify, EquModifyStatus } from '@/api/DeviceData'
const fixHeight = 260
export default {
  name: 'IncomingInsp',
  ...
  data() {
    return {
      ...
      rules: {
        ...
        MtItemsName: [{ required: true, message: '请输入设备保养型名称', trigger: 'blur' }]
      },
  ...
  methods: {
    ...
    handleBan(row) {
      ...
      this.$confirm(statusTitle, status, {
        ...
        const params = {
          ...
          MtItemsCode: row.MtItemsCode
        }
        EquModifyStatus(params).then(res => {
          if (res.IsPass === true) {
            this.$message({
              type: 'success',
              message: res.MSG
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.dialogType === 'edit') {
            EquMtItemsModify(this.ruleForm).then(res => {
              if (res.IsPass === true) {
                this.$message({
                  type: 'success',
            ...
            EquMtItemsAdd(this.ruleForm).then(res => {
              if (res.IsPass === true) {
                this.$message({
                  type: 'success',
    ...
    handleDelete(row) {
      this.$confirm(this.$t('permission.errorInfo'), this.$t('permission.errorTitle'), {
        ...
        .then(() => {
          EquMtItemsDelete({ MtItemsCode: row.MtItemsCode }).then(res => {
            if (res.IsPass === true) {
              this.$message({
                type: 'success',
