  ...
  <div class="app-container">
    <div class="search">
      <el-row :gutter="20">
        <el-col :span="6">
          <el-col :span="8">
            <el-tooltip class="item" effect="dark" :enterable="false" content="设备类型编号" placement="top-start"><label class="radio-label">设备类型编号:</label></el-tooltip>
          </el-col>
          <el-col :span="16"><el-input v-model.trim="pagination.EquTypeNum" placeholder="设备类型编号" clearable /></el-col>
        </el-col>

        <el-col :span="6">
          <el-col :span="8">
            <el-tooltip class="item" effect="dark" :enterable="false" content="设备类型名称" placement="top-start"><label class="radio-label">设备类型名称:</label></el-tooltip>
          </el-col>
          <el-col :span="16"><el-input v-model.trim="pagination.EquTypeName" placeholder="设备类型名称" clearable /></el-col>
        </el-col>
    ...
    >
      ...
      <el-table-column align="center" label="设备类型编号" width="150" prop="EquTypeNum" sortable :show-overflow-tooltip="true">
        <template slot-scope="scope">
          {{ scope.row.EquTypeNum }}
        </template>
      </el-table-column>

      <el-table-column align="center" label="设备类型名称" width="150" prop="EquTypeName" sortable :show-overflow-tooltip="true">
        <template slot-scope="scope">
    ...
    >
      <el-form ref="ruleForm" v-loading="editLoading" :model="ruleForm" :rules="rules" label-width="120px" label-position="left">
        <el-form-item label="设备类型编号" prop="EquTypeNum"><el-input v-model.trim="ruleForm.EquTypeNum" placeholder="设备类型编号" clearable /></el-form-item>
        <el-form-item label="设备类型名称" prop="EquTypeName"><el-input v-model.trim="ruleForm.EquTypeName" placeholder="设备类型名称" clearable /></el-form-item>
        ...
        <el-form-item label="预警产量" prop="PreAlertTimes"><el-input v-model.trim="ruleForm.PreAlertTimes" placeholder="预警产量" clearable /></el-form-item>
...
import { EquBaseList, EquBaseAdd, EquBaseDelete, EquBaseModify, EquBaseModifyStatus } from '@/api/DeviceData'
const fixHeight = 260
export default {
  name: 'IncomingInsp',
  ...
  data() {
    return {
      ...
      pagination: {
        PageIndex: 1,
        PageSize: 30,
        EquTypeNum: undefined,
        EquTypeName: undefined,
        ShowBanned: false
      },
      listLoading: false,
      ...
      rules: {
        EquTypeNum: [{ required: true, message: '请输入设备类型编号', trigger: 'blur' }],
        EquTypeName: [{ required: true, message: '请输入设备类型名称', trigger: 'blur' }]
      },
  ...
  methods: {
    ...
    handleBan(row) {
      ...
      this.$confirm(statusTitle, status, {
        ...
        EquBaseModifyStatus(params).then(res => {
          if (res.IsPass === true) {
            this.$message({
              type: 'success',
              message: res.MSG
    ...
    getList() {
      ...
      EquBaseList(this.pagination).then(res => {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.dialogType === 'edit') {
            EquBaseModify(this.ruleForm).then(res => {
              if (res.IsPass === true) {
                this.$message({
                  type: 'success',
            ...
            EquBaseAdd(this.ruleForm).then(res => {
              if (res.IsPass === true) {
                this.$message({
                  type: 'success',
    ...
    handleDelete(row) {
      this.$confirm(this.$t('permission.errorInfo'), this.$t('permission.errorTitle'), {
        ...
        .then(() => {
          EquBaseDelete({ EquTypeCode: row.EquTypeCode }).then(res => {
            if (res.IsPass === true) {
              this.$message({
                type: 'success',
