  ...
  <div class="app-container">
    <div class="search">
      <el-row :gutter="20">
        ...
        <el-col :span="5">
          ...
          <el-col :span="16"><el-input v-model.trim="pagination.TypeNum" placeholder="类型编号" clearable /></el-col>
        </el-col>
        ...
        <el-col :span="5">
          ...
          <el-col :span="16"><el-input v-model.trim="pagination.Name" placeholder="类型名称" clearable /></el-col>
        </el-col>
    ...
    <el-dialog v-dialogDrag :close-on-click-modal="false" :visible.sync="dialogFormVisible" :title="dialogType === 'edit' ? $t('permission.EditCompany') : $t('permission.addCompany')">
      <el-form ref="ruleForm" v-loading="editLoading" :model="ruleForm" :rules="rules" label-width="120px" label-position="left">
        <el-form-item label="类型编号" prop="TypeNum"><el-input v-model.trim="ruleForm.TypeNum" placeholder="类型编号" clearable /></el-form-item>

        <el-form-item label="类型名称" prop="Name"><el-input v-model.trim="ruleForm.Name" placeholder="类型名称" clearable /></el-form-item>

        <el-form-item label="描述" prop="Description"><el-input v-model.trim="ruleForm.Description" placeholder="描述" type="textarea" /></el-form-item>
...
import { MaterialTypeList, MaterialTypeAdd, MaterialTypeDelete, MaterialTypeModify, MaterialTypeModifyStatus } from '@/api/BasicData'
const fixHeight = 260
export default {
  name: 'IncomingInsp',
  ...
  data() {
    return {
      ...
      rules: {
        TypeNum: [{ required: true, message: '请输入类型编号', trigger: 'blur' }],
  ...
  watch: {
    ...
    'ruleForm.TypeNum': function(val) {
      if (val === '' || val === undefined) {
        return
      } else {
        this.ruleForm.TypeNum = this.filterInput(val)
      }
    },
  ...
  methods: {
    ...
    handleBan(row) {
      ...
      this.$confirm(statusTitle, status, {
        ...
        MaterialTypeModifyStatus(params).then(res => {
          if (res.IsPass === true) {
            this.$message({
              type: 'success',
              message: res.MSG
    ...
    getList() {
      ...
      MaterialTypeList(this.pagination).then(res => {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          if (this.dialogType === 'edit') {
            MaterialTypeModify(this.ruleForm).then(res => {
              if (res.IsPass === true) {
                this.$message({
                  type: 'success',
            ...
            MaterialTypeAdd(this.ruleForm).then(res => {
              if (res.IsPass === true) {
                this.$message({
                  type: 'success',
    ...
    handleDelete(row) {
      this.$confirm(this.$t('permission.errorInfo'), this.$t('permission.errorTitle'), {
        ...
        .then(() => {
          MaterialTypeDelete({ TypeCode: row.TypeCode }).then(res => {
            if (res.IsPass === true) {
              this.$message({
                type: 'success',
